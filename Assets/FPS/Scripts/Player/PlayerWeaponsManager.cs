...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    public List<BasicWeapon> startingWeapons = new List<BasicWeapon>();
    ...
    public UnityAction<BasicWeapon> onSwitchedToWeapon;
    public UnityAction<BasicWeapon, int> onAddedWeapon;
    public UnityAction<BasicWeapon, int> onRemovedWeapon;

    private Interact inter;

    BasicWeapon[] m_WeaponSlots = new BasicWeapon[9]; // 9 available weapon slots
    ...
    void UpdateWeaponSwitching()
    {
        ...
        if(switchingTimeFactor >= 1f)
        {
            if (m_WeaponSwitchState == WeaponSwitchState.PutDownPrevious)
            {
                ...
                BasicWeapon oldWeapon = GetWeaponAtSlotIndex(activeWeaponIndex);
                if (oldWeapon != null)
                {
                ...
                // Activate new weapon
                BasicWeapon newWeapon = GetWeaponAtSlotIndex(activeWeaponIndex);
                if (onSwitchedToWeapon != null)
                {
                    onSwitchedToWeapon.Invoke(newWeapon);
                }
    ...
    public BasicWeapon GetWeaponAtSlotIndex(int index)
    {
